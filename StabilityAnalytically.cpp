...
StabilityAnalytically::StabilityAnalytically()
            :generator(time(0)), distribution(3), distr(generator, distribution)
{
    //ctor
    NearZero = 1e-15;
    NearSphere = 1e-5;
...
void StabilityAnalytically::startTask()
{
    ...
    //std::cerr<<"E3\n";
    //std::cin.get();
    std::cerr<<"Calculate Stiffness:...";
    calculateActualStiffness();
    std::cerr<<"OK!"<<std::endl;
    std::cerr<<"reach Stress...";
    reachSpecifiedStress();
    std::cerr<<" OK!"<<std::endl;
/*
    StabilityPointType Pdvar;
    boost::qvm::mat<double,3,3> defGrad=MC_0T33;
    ...
    checkStability(Pdvar);

    std::cerr<<"Stab "<<int(Pdvar.Stability)<<"\n";/**/
    //std::cerr<<"E4\n";
    // std::cin.get();
}
...
void StabilityAnalytically::checkStability3D(boost::qvm::vec<double,3> *Pvar,
    ...
    std::ofstream Res_File ("Res_file.txt");
    std::ofstream Res_Stab_File ("Res_Stab_file.txt");
    for(uint_fast32_t i=0; i<Nvar; ++i){
...
void StabilityAnalytically::checkStability(StabilityPointType &Pd)
{
    ...
    for(uint_fast32_t i=1; i<RSN; ++i)
    {
        ...
        // if(fabs(P1[i])>near_zero || fabs(P2[i])>near_zero)
        //     std::cerr<<"Q "<<i<<" "<<A[i]<<" "<<P1[i]<<" "<<_1d_A[i]<<" "<<P2[i]<<"\n";
    }
    // std::cerr<<"QQ "<<Q.a[0][0]<<" "<<Q.a[0][1]<<" "<<Q.a[1][0]<<"\n";
...
void StabilityAnalytically::calculateForces()
{
    ...
    for (uint_fast32_t i=1; i<RSN; ++i)
    {
        ...
        // std::cerr<<"Stress "<<_1d_V<<" "<<A[i]<<" "<<P1[i]
        //             <<" "<<Stress.a[0][0]<<" "
        //     <<Stress.a[0][1]<<" "<<Stress.a[1][0]<<" "<<Stress.a[1][1]<<"\n";

    }
    // std::cerr<<"Stress "<<Stress.a[0][0]<<" "<<Stress.a[1][1]<<" "<<Stress.a[2][2]<<"\n";
    if(isnan(Stress.a[0][0]))
            std::cin.get();
    // std::cerr<<"Stress "//<<_1d_V*A[i]*P1[i]
    //             <<" "<<Stress.a[0][0]<<" "
    //     <<Stress.a[0][1]<<" "<<Stress.a[1][0]<<" "<<Stress.a[1][1]<<"\n";
...
void StabilityAnalytically::findCoordinationalSpheres()
{
    ...
    for(uint_fast32_t i=0; i<N; ++i)
    {
        ...
        //std::cerr<<"R "<<boost::qvm::X(R[i]-R[iCenter])<<" "
        // <<boost::qvm::Y(R[i]-R[iCenter])<<" "<<boost::qvm::Z(R[i]-R[iCenter])
        // <<" "<<dr_mm<<"\n";
        //std::cin.get();
...
//std::cerr<<"Q3 "<<VectorNumber<<"\n";
    ...
    for(uint_fast32_t i=0; i<N; ++i)
    {
        ...
        //std::cerr<<"SVp "<<i<<" "<<dr_mm<<" "<<boost::qvm::X(R[i]-R[iCenter])
        // <<" "<<boost::qvm::Y(R[i]-R[iCenter])<<"\n";
...
void StabilityAnalytically::decomposeVectors()
{
    for (uint_fast32_t i=0; i<RSN; ++i)
    {
        ...
        //std::cerr<<"DV "<<RS[i].a[0]<<" "<<RS[i].a[1]<<" "<<RS[i].a[2]<<" "<<Ain[i]-1.0<<"\n";
        _1d_A[i] = 1.0/Ain[i];
        ein[i]   = RS[i]*_1d_A[i];
    }
...
//     // n.a[0] = 40;
//     ai::printMarker(100);
//     std::cout<<"n.a[0] = "<<_Nonnull(n.a)<<std::endl;//<<n.a[0];//_Nonnull(n.a)<<std::endl;
//     ai::printMarker();
//     n.a[1] = n.a[0];
//     ai::printMarker();
...
void StabilityAnalytically::createFCCLattice()
{
    ...
    // n.a[0] = 40;
    // ai::printMarker(100);
    // std::cerr<<"."<<n.a[0];//<<n.a[0];//_Nonnull(n.a)<<std::endl;
    // ai::printMarker();
    // n.a[1] = n.a[0];
    // ai::printMarker();
    // std::cerr<<"."<<std::endl;
...
//std::cerr<<"R5\n";
    ...
    // std::cerr<<"."<<std::endl;
    uint_fast32_t i, j, k, iN = 0;
    // ai::printMarker(500000000000000);
    for(k=0; k<n.a[2]; ++k)
        for(j=0; j<n.a[1]; ++j)
            for(i=0; i<n.a[0]; ++i){
                if((i%2+j%2+k%2)%2)
    ...
    std::cerr<<"."<<std::endl;
        //std::cerr<<"C "<<N<<" "<<eStep.a[0]<<" "<<eStep.a[1]<<" "<<eStep.a[2]<<"\n";
    std::cerr<<"FCC!!!!!!!!! = "<<" N = "<<N<<" V = "<<V<<" iCenter = "<<iCenter<<"\n";
    std::cerr<<"OK!"<<std::endl;
    //std::cin.get();
}
...
void StabilityAnalytically::reachSpecifiedStress(const boost::qvm::mat<double,3,3> &Svar)
{
    ...
    for(;;)
    {
        ...
        // std::cerr<<".";
        dS=Stress-Svar;
        std::cout<<"dS  = "<<dS.a[0][0]<<" "<<dS.a[1][1]<<std::endl;
